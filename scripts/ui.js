...
export function init(){
 ...
 if (sortScore) sortScore.addEventListener('click', function(e) {
    e.stopPropagation(); 
    sortRanking('score');
  });

 var sortTime = document.getElementById('sort-by-time');
 if (sortTime) sortTime.addEventListener('click', function(e) {
    e.stopPropagation(); 
    sortRanking('time');
  });

 var sortDate = document.getElementById('sort-by-date');
 if (sortDate) sortDate.addEventListener('click', function(e) {
    e.stopPropagation(); 
    sortRanking('date');
  });
...
function openRanking(){
 ...
 lastRankingArray = (Storage.loadResults() || []).map(it => ({
  name: it.name || '',
...
return;
 ...
 renderRankingList(lastRankingArray);
 modal.classList.remove('hidden');
 play(audioOpen);
}
...
export function sortRanking(key){
 ...
 lastRankingArray.sort(function(a,b){
  if (key === 'date') {
   return rankingSortState.date *
